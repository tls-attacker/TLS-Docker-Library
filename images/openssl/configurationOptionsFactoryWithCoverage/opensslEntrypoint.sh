...
then
    ...
    /bin/openssl-server-entrypoint ./apps/openssl s_server -accept 4433 -key /cert/ec256key.pem -cert /cert/ec256cert.pem -comp -cipher ALL &
    child=$!
    wait "$child"
    wait "$child"
...
then
    ...
    /bin/openssl-client-entrypoint ./apps/openssl s_client -connect $client_dest -comp -cipher ALL &
    child=$!
    wait "$child"
    wait "$child"
fi
