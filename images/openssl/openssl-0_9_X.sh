...
if [ ! -z "$DOCKER_REPOSITORY" ]; then
	docker push ${DOCKER_REPOSITORY}openssl-server:0.9.7
	docker push ${DOCKER_REPOSITORY}openssl-client:0.9.7
fi
...
do
	...
	if [ ! -z "$DOCKER_REPOSITORY" ]; then
		docker push ${DOCKER_REPOSITORY}openssl-server:0.9.7${array7[$i]}
		docker push ${DOCKER_REPOSITORY}openssl-client:0.9.7${array7[$i]}
	fi
	i=i+1
done
...
do
	...
	if [ ! -z "$DOCKER_REPOSITORY" ]; then
		docker push ${DOCKER_REPOSITORY}openssl-server:0.9.8${array8[$i]}
		docker push ${DOCKER_REPOSITORY}openssl-client:0.9.8${array8[$i]}
	fi
	i=i+1
done
...
_docker build --build-arg VERSION=8m-beta1 -t ${DOCKER_REPOSITORY}openssl-client:0.9.8m_beta1 -f Dockerfile-0_9_8m-zh --target openssl-client .
	if [ ! -z "$DOCKER_REPOSITORY" ]; then
		docker push ${DOCKER_REPOSITORY}openssl-server:0.9.8m_beta1
		docker push ${DOCKER_REPOSITORY}openssl-client:0.9.8m_beta1
	fi
