...
def ct_lsb_prop_u32(val):
    """Propagate LSB to all 32 bits of the returned int. Constant time."""
    val &= 0x01
    val |= val << 1
    val |= val << 2
    val |= val << 4
...
def ct_is_negative_u32(val):
    return (0x100000000 & val) >> 32

    
def ct_max_32_not_both_neg(val_a, val_b):
    # all 1 if a >= b, 0 otherwise
    mask_a = ct_lsb_prop_u32(1 ^ ct_lt_u32(val_a, val_b))
    # all 1 if b >= a, 0 otherwise
    mask_b = ct_lsb_prop_u32(1 ^ ct_lt_u32(val_b, val_a))

    a_is_negative = ct_is_negative_u32(val_a)
    b_is_negative = ct_is_negative_u32(val_b)
    
    force_b = ct_lsb_prop_u32(a_is_negative & (1 ^ b_is_negative))
    force_a = ct_lsb_prop_u32(b_is_negative & (1 ^ a_is_negative))
    none_forced = 0xFFFFFFFF ^ (force_a | force_b)
    return (force_a & val_a) | (force_b & val_b) | (none_forced & ((mask_a & val_a) | (mask_b & val_b)))
...
def ct_check_cbc_mac_and_pad(data, mac, seqnumBytes, contentType, version,
    ...
    pad_start = ct_max_32_not_both_neg(0, pad_start)
    ...
    if version == (3, 0): # version is public
        ...
        start_pos = ct_max_32_not_both_neg(0, data_len - 256)
    ...
    mac_start = ct_max_32_not_both_neg(0, mac_start)

    # place to start processing
    start_pos = ct_max_32_not_both_neg(0, data_len - (256 + mac.digest_size)) // mac.block_size
