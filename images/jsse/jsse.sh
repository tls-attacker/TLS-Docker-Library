...
do
	...
	if [ ! -z "$DOCKER_REPOSITORY" ]; then
		docker push ${DOCKER_REPOSITORY}jssetls_jre-server:${jreversions[$i]}_bc_1_${bcversion}
	fi
	i=i+1
done
...
if [ ! -z "$DOCKER_REPOSITORY" ]; then
	docker push ${DOCKER_REPOSITORY}jssetls_jre-server:${jreversion}_bc_1_${bcversion}
fi
